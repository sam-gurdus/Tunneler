$(function(){
    ...
    var refreshButton = $('h1 .icon-refresh'),
        ...
        icd = $('#icd'),
        siteurl = '/pages/chat'; // Replace dot with the ChatX URL if you're gonna use chat on other pages
    ...
    var time = checkInt ? 500 : 3000;

    // Automatically refresh the shouts every 3 seconds or on fast, .5 second
    loadInt = setInterval(load,time);

    icd.on('change', function(){
        clearInterval(loadInt);
        loadInt = setInterval(load,($(this).prop('checked') ? 500 : 3000));
...
*/
    ...
    // Hide settings window when clicked off
    ...
    //By default, the two position tags below should be set to fixed
