...
if(isset($_POST['url'])){
	...
	if(Config::get('index_redirect')){
		...
		echo render_template("proxy/home.html", array('version' => Proxy::VERSION));
	}
...
foreach(Config::get('plugins', array()) as $plugin){
	...
	if(file_exists('proxy/plugins/'.$plugin_class.'.php')){

		// use user plugin from /plugins/
		require_once('proxy/plugins/'.$plugin_class.'.php');
...
try {
	...
	if(Config::get("error_redirect")){
		...
		echo render_template("proxy/home.html", array(
			'url' => $url,
